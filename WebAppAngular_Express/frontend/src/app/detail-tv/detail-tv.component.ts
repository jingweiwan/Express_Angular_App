...
@Component({
  ...
  styleUrls: ['./detail-tv.component.css'],
...
export class DetailTVComponent implements OnInit {
  ...
  obj_tv_cast_list_mobile:any=[];
  obj_similar_movie_list:any=[];
  obj_tv_cast_list:any=[];
  cast_list:any=[];
  ...
  ngOnInit(): void {
    ...
    this.appService.getTVDetail(this.id).subscribe(data=>{
      this.movie_results = data;
      let x = JSON.parse(this.storage.getItem("continue"));
      let inArray=false;
      
      this.movie_title = this.movie_results.name;
      ...
      this.release_date = this.movie_results.first_air_date.substring(0,4);
      this.runtime_hours = Math.floor(this.movie_results.episode_run_time/60);
      this.runtime_minutes = this.movie_results.episode_run_time%60;
    ...
    this.appService.getTvVideo(this.id).subscribe(data=>{
    ...
    //recommend TV
    this.appService.getRecommendTV(this.id).subscribe(data=>{
    ...
    //similar TV
    this.appService.getSimilarTV(this.id).subscribe(data=>{
    ...
    //TV cast
    this.appService.getTVcast(this.id).subscribe(data=>{
      this.tv_cast_results = data;
      
      for(let i = 0;i<this.tv_cast_results.cast.length;i++){
        if(this.tv_cast_results.cast[i].profile_path){
            this.cast_list.push(this.tv_cast_results.cast[i]);
        }
      }
    ...
    this.appService.getTVreview(this.id).subscribe(data=>{
      ...
      for(let i=0; i<this.movie_review_results.results.length;i++){
        if(this.review_path_list.length<10){
          ...
          if(!this.movie_review_results.results[i].author_details.rating){
            this.movie_review_results.results[i].author_details.rating=0;
          }
        }
  ...
  open(content:any,actor_id:any,actor_n:any){
    ...
    this.appService.getCastDetail(actor_id).subscribe(data=>{
      ...
      this.imdb=this.cast_detail_results.imdb_id;
